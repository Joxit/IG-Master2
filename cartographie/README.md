...
- Besoin de faire des calculs géospatiaux ? Allez voir [Turf.js](https://turfjs.org/)

Vous voulez utiliser autre chose ? Demandez mon avis ;)

## Restrictions backend

- Les langages autorisés sont JVM (Java et Kotlin), NodeJS (Javascript et TypeScript) et Rust (pour le fun)
...
Jawg Maps est sponsor de l'application StreetComplete depuis 2020 où nos fournissons gratuitement le fond de carte au créateur de l'application. C'est un projet open source que vous pouvez retrouver sur [GitHub.com/StreetComplete](https://github.com/streetcomplete/StreetComplete/).
...
- Il ne peut voir qu'une seule énigme à la fois
...
- L'utilisateur doit se déplacer sur la carte pour trouver l'emplacement de l'énigme
  - Le point doit donc être caché jusqu'à un certain moment avant d'être révélé, vous avez 2 choix:
    ...
    - Vous pouvez demander à l'utilisateur de cliquer sur la carte
      - Au clic, un POI temporaire sera affiché pour montrer le choix de l'utilisateur
      - Le POI peut être déplacé (drag and drop) ou supprimé (clic dessus) ou écrasé (clic sur une autre zone)
...
- Sauvegarder l'historique des actions de votre utilisateur
  - Il devra y avoir un historique par étape => L'énigme 1 a trois actions, l'énigme 2 a quatre actions, etc
...
- Utiliser une base de donnée qui permet d'effectuer des calculs géospatiaux (comme Postgis)
- Les positions et informations doivent être enregistrés dans la BDD
...
- Vous devez ajouter des interactions sur les marker
  - Au clic ouvrez une popup et affichez des informations tirées de la donnée OSM
